function mixsamp = voximg2ksppar(imPall,cmap,nval,opts,kz)
% Convert phantom images to k-space partition water only
%
%   INPUTS:
...
%       kz     [1X1]            -> The partition of interest in kspace
%
%   OUTPUT:
%       mixsamp [nx, ny, nc] -> 4D phantom in k-space with phase accumulation and noise
%
% -----------------------------------------------------------------------------------------
...
for c = 1:nc
    if strcmp(opts.trajectory,'cartesian')
        ...
        %         for ipar = 1:npar
        ksp2DW(:,:) = embed(opts.G*kspW(:,:,c,kz),im2kspmask);
    ...
    mixksp = ksp2DW; % water only
    
    % Add noise
    mixsamp(:,:,c) = mixksp+nval*(randn(nr,np)+1i*randn(nr,np));
end
